...
export default function TableInCompletePaid() {
  ...
  const fetchData = useCallback(async (pageIndex, pageSize, filterSearch) => {
    try {
      ...
      const url = `${import.meta.env.VITE_BACKEND_URL}customers/incomplete-paying/?page=${apiPageNumber}&page_size=${pageSize}&search=${filterSearch}`;
...
/* old progress bar i don't need it now   
    {
      accessorFn: (row) => {
