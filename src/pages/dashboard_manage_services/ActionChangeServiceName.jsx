...
import changeServiceName from '../../services/changeServiceName';



export default function ActionChangeServiceName({ service, openModal, onClose, onServiceNameUpdate }) {
  ...
  const [modalTitle, setModalTitle] = useState("Changer le nom de service");
  ...
  //check if the service name are not the same as the current ones
  if (data.service_name.trim() === service.service_name) {
    setError("service_name", {
      type: "manual",
      message: "Le nom du service doit être différent de l’actuel.",
    });
    return;
  }
  ...
  try {
    const response = await changeServiceName(requestData);
    ...
    if (response) {
      ...
      onServiceNameUpdate({
        id: service.id,
        service_name: response.new_service_name,
      ...
      >
          ...
          render={({ field, fieldState}) => (
            <TextField
              ...
              defaultValue={service.service_name || ''} // Set default value from service object
            />
          )}
          >
