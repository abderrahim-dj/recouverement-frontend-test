...
export default function ChartClient() {
  ...
  const fetchData = async (selectedDate, filterBy) => {
    ...
    try {
      const url = `${import.meta.env.VITE_BACKEND_URL}customers-debt-status/?date=${selectedDate}&calculate=${filterBy}`;
      ...
      alert("Error fetching data from API IIIII");
    }
...
const chartData =
  filteredData.length > 0
    ? {
        labels:
          filterType === "month"
            ? filteredData.map((item) => format(parseISO(item.date), "dd MMM")) // show day and month
            : filteredData.map((item) => format(parseISO(item.date), "MMM")),   // show month
        datasets: [
          {
            label: "Avec Dette",
            data: filteredData.map((item) => item.has_dette),
            borderColor: "#FF6384",
          ...
          {
            label: "Sans Dette",
            data: filteredData.map((item) => item.has_not_dette),
            borderColor: "#36A2EB",
  ...
  return (
   <div style={{ maxWidth: 700, margin: "0 auto", boxShadow: "5px 10px 8px rgba(0,0,0,0.1)", padding: "20px", borderRadius: "8px", width:'40vw' }}>
      <h2 className="text-2xl font-bold pb-6 pl-8">Historique des dettes</h2>
      ...
      {chartData ? 
        ...
        : 
        (<div style={{ textAlign: "center", color: "#888" }}>Aucune donnée à afficher</div>)  
      }
    </div>
  );
}